...
if openai_api_key and openai_api_key != 'your_openai_api_key_here':
    try:
        ...
        print(f"Error initializing OpenAI client or quota exceeded: {e}")
...
def analyze_skill_gap(resume_text, job_role):
    """
    Analyze the resume text and job role to identify skill gaps.
    Returns a list of skills to be learned.
    """
    # Placeholder logic for skill gap analysis
    # In a real implementation, this would involve NLP and domain-specific analysis
    skills_in_resume = set(re.findall(r'\b\w+\b', resume_text.lower()))
    required_skills = {
        'data analyst': {'python', 'sql', 'statistics', 'data visualization'},
        'software engineer': {'java', 'c++', 'algorithms', 'data structures'}
    }
    
    job_role_key = job_role.lower()
    if job_role_key in required_skills:
        missing_skills = required_skills[job_role_key] - skills_in_resume
...
def generate_interview_questions(resume_text, job_role, experience_level, num_questions=4):
    try:
        ...
        # Analyze skill gap
        skills_to_learn = analyze_skill_gap(resume_text, job_role)
        print(f"Skills to be learned: {skills_to_learn}", file=sys.stderr)
...
def progress_updates():
    def event_stream():
        # This would be connected to your database or pub/sub system
        while True:
            # Check for updates in the database
            updates = check_for_progress_updates()  # You'll need to implement this
            if updates:
...
def check_for_progress_updates():
    # Implement logic to check for progress changes
    # Example: Query database for recently updated goals
    # Return format: {'goal_id': 123, 'new_progress': 75}
    pass
...
def complete_task():
    ...
    # After completing task, update goal progress
    goal_id = get_goal_for_task(task_id)  # Implement this
    new_progress = calculate_updated_progress(goal_id)  # Implement this
    
    # This would trigger the SSE update
    store_progress_update(goal_id, new_progress)  # Implement this
...
def ai_summarize_text(text, method='auto'):
    """
    Summarize text using available AI services with fallback to NLTK.
    
    Args:
        text (str): Text to summarize
        method (str): 'auto' (try OpenAI then Gemini then NLTK), 
                     'openai', 'gemini', or 'nltk'
    Returns:
        str: Summary text
    """
    # First try OpenAI if available and requested
    if (method == 'auto' or method == 'openai') and openai_available:
        try:
            ...
            if method == 'openai':  # If specifically requested, don't fall back
                return "OpenAI summarization failed"

    # Then try Gemini if available
    if (method == 'auto' or method == 'gemini') and gemini_model:
        try:
    ...
    # Fall back to NLTK extractive summarization
...
def summarize_pdf():
    ...
    if 'pdf_text' not in session or not session['pdf_text']:
        return jsonify({'success': False, 'error': 'No text to summarize'})
    
    try:
        summary = ai_summarize_text(session['pdf_text'])  # Changed from summarize_text to ai_summarize_text
        return jsonify({'success': True, 'summary': summary})
    except Exception as e:
        print(f"Summarization error: {e}")
        return jsonify({'success': False, 'error': str(e)})

# Remove the duplicate summarize_pdf route that appears later in the file
# ... rest of the file ...
...
def practice():
    ...
    try:
        # Try to import openpyxl if not already available
        import openpyxl
        # Load the Excel file with problem set data
        xlsx_path = 'data/Strivers.xlsx'
        df = pd.read_excel(xlsx_path)
    except ImportError:
        flash("Please install openpyxl package to load problem data: pip install openpyxl", "error")
        return redirect(url_for('dashboard'))
    except Exception as e:
        flash(f"Error loading problem set data: {str(e)}", "error")
        return redirect(url_for('dashboard'))
    
    # ... rest of your practice route code ...
